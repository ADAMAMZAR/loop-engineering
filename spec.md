...
A task may have a `- verify: \`command\`` line right after it. That command
is run by the harness itself after the agent finishes, and a task is only
checked off and committed if it exits 0. The agent's own claim of "I tested
it and it works" is not trusted on its own — this is the actual gate.
...
- [x] Create `greet.py` with a function `greet(name)` that returns
  ...
  - verify: `python -c "from greet import greet; assert greet('World') == 'Hello, World!'"`
- [x] Create `math_ops.py` with functions `add(a, b)` and `multiply(a, b)`.
  ...
  - verify: `python test_math_ops.py`
